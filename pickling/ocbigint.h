...
template<class I, class BI>
inline BigUInt<I,BI> MakeBigUIntFromBigInt (const BigInt<I,BI>& s);
...
class BigInt {
  ...
  friend BigUInt<I,BI> MakeBigUIntFromBigInt<> (const BigInt& s);
 ...
 public:
  ...
  // Take and make
  BigInt (int_8 i, Allocator* a=0):
    impl_( i<0 ? int_u8(-i) : int_u8(i), a)
  {
    sign() = i<0 ? 1 : 0;
  }
 ...
 protected:
  ...
  // sign embedded in impl (array) so as to keep class under 32 bytes
...
template<class I, class BI>
inline BigUInt<I,BI> MakeBigUIntFromBigInt (const BigInt<I,BI>& s) 
{ BigUInt<I, BI> result(s.impl_); result.sign()=0; return result; }
...
inline int_n StringToBigInt (const char* data, int len)
{
  int_n result;  // RVO
  char c=' ';
  char sign = '\0';
...
inline int_n StringToBigInt (const char* data)
{ return StringToBigInt(data, strlen(data)); }

inline int_n StringToBigInt (const string& s)
{ return StringToBigInt(s.data(), s.length()); }
