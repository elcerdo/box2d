			    ...
			    MachineRep_e endian=MachineRep_EEEI)
{
  ...
  for (int ii=0; ii<int(entries); ii++) {
    ...
    if (convert_to_arr && conversion_to_arr_still_possible) {
      conversion_to_arr_still_possible = Stringize(ii)==string(key);
    }
  }
  // If we can convert to an Arr and we want to, do it!
  if (entries &&  // prefer empty to be tables (just python pickling proto 0)
      conversion_to_arr_still_possible && convert_to_arr) {
    Arr a(entries);
    for (int ii=0; ii<int(entries); ii++) {
      a.append(t[Stringize(ii)]);
    }
    v = a;
  }
  return mem;
