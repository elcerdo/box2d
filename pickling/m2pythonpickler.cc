...
template <class OBJ> void PythonDepicklerA<OBJ>::pPUT_ ()
{
  ...
  // Get the top of the PM Context Stack: that's what we want to
  // memoize.  We can keep it as a pointer until it actually gets
  // popped from the stack, at which point we have to record it
  // (copy).
  int index_of_entry = stack_.length()-1;
  PStack_<OBJ>& p = stack_[index_of_entry];
  p.memo_number = memo_num; // Record the memo number so pops correctly: This is the 1-based memo
  ...
  // Remember where (0-based, not 1-based, like addr) we "shared" this string
  for (int ii=memo_.length(); ii<memo_num-1; ii++) {
    memo_.append(MemoInfo_<OBJ>(0));
  }
  memo_.append(MemoInfo_<OBJ>(&p.object));  
}
...
template <class OBJ> void PythonDepicklerA<OBJ>::pGET_ ()
{
  int len;
  char* ascii_num = getUntilNewLine_(len);
  int_4 addr = atoi(ascii_num)-1; // Translate 1-based to 0-based
  MemoInfo_<OBJ>& m = memo_[addr]; 
  OBJ* shared = m.object_ptr;
  push_(*shared);
}
...
template <class OBJ> void PythonDepicklerA<OBJ>::ploadTUPLE_ ()
{
  ...
  int tuple_length = stack_.length() - tuple_start;
  int stack_len = stack_.length();
  for (int ii=tuple_start; ii<stack_len; ii++) {
    TupleAppend(tuple, stack_[ii].object);
  }
